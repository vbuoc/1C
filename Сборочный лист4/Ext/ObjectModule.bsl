...
функция ПечатьНаПогрузку()
	...
	//+1Ctr@, 02.02.2022 12:36:18 29121
	СвязанныеДокументы = КритерииОтбора.СвязанныеДокументы.Найти(ЭтотОбъект.СсылкаНаОбъект);
	//+1Ctr@, 02.02.2022 12:36:18
	Для Каждого ВыборкаСтрок Из ЗапросТовары Цикл
		//+1Ctr@, 02.02.2022 12:36:18 29121
		КомплектЕстьВДК = Ложь;
		Для  Каждого СтрДокКомплектации Из СвязанныеДокументы Цикл
			Если НЕ СтрДокКомплектации.ПометкаУдаления И СтрДокКомплектации.Проведен И ТипЗнч(СтрДокКомплектации) = Тип("ДокументСсылка.КомплектацияНоменклатуры") Тогда
				Если СтрДокКомплектации.Номенклатура = ВыборкаСтрок.Номенклатура Тогда
					ДКСсылка = СтрДокКомплектации;
					КомплектЕстьВДК = Истина;
				КонецЕсли;	
			КонецЕсли;	
		КонецЦикла;	
		//+1Ctr@, 02.02.2022 12:36:18
		...
		Если ВыборкаСтрок.Номенклатура.ИТА_Комплект Тогда
			...
			Если СтрокиКомплектации.Количество() Тогда
				...
				Для Каждого СтрКомпл Из СтрокиКомплектации Цикл
					//+1Ctr@, 02.02.2022 12:36:18 29121
					Если КомплектЕстьВДК Тогда
						ПараметрыОтбора = Новый Структура;
						ПараметрыОтбора.Вставить("Номенклатура", СтрКомпл.Комплектующая);
						НайденноеЗначение = ДКСсылка.Комплектующие.НайтиСтроки(ПараметрыОтбора);
						Если НайденноеЗначение.Количество() = 0 Тогда
							ПараметрыОтбора = Новый Структура;
							ПараметрыОтбора.Вставить("НоменклатураЗаменаОсновнойКомплектации", СтрКомпл.Комплектующая);
							НайденноеЗначение = ДКСсылка.Комплектующие.НайтиСтроки(ПараметрыОтбора);
							Если НайденноеЗначение.Количество() = 0 Тогда							
								Продолжить;
							КонецЕсли;
